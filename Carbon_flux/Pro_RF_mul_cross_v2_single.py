...
if __name__ == "__main__":
    ...
    np.save(results_variation_path + 'RF_RLM_NEE_x_mask_noise.npy', x_inter_mask_array)
    np.save(results_variation_path + 'RF_RLM_NEE_y_qa_noise.npy', y_flux_qa_array)
    # 将质量标识的波段去除
    ...
    for kth_data in cross_validation_data:
        ...
        # 没有soil water
        RFmodel.fit(RF_input_images_train_norm4_clean[:,:], RF_trainy_input)
        # r_score = RFmodel.score(RF_input_images_test_norm4_clean[:, :9], RF_testy_input)
        # print(r_score)
        y_pred = RFmodel.predict(RF_input_images_test_norm4_clean[:, :])
    ...
    np.save(results_variation_path + 'RF_RLM_NEE_predict_noise.npy', Y_pred_filtered_array)
    np.save(results_variation_path + 'RF_RLM_NEE_true_noise.npy', Y_test_filtered_array)
    np.save(results_variation_path + 'RF_RLM_NEE_site_noise.npy', RF_test_sites_array)
