...
#     # 第一组值:插值与不插值的一起
    ...
    for i in range(8):
        ...
        density_values = hist.ravel()  # Flatten histogram values

        exclude_percentile = 20  # Change this to the desired percentage
        # Calculate the density threshold at the given percentile
        density_threshold = np.percentile(density_values[density_values > 0], exclude_percentile)

        h = axs[row, col].hist2d(x, y, bins=bins_n, cmap=cmap,norm=Normalize(vmin=density_threshold))  # Adjust bins for density granularity

        text = f"RMSE = {rmse:.2f}\nCC = {cc:.2f}"
        axs[row, col].text(0.05, 0.97, text, verticalalignment='top', transform=axs[row, col].transAxes,
                           color='red', fontweight='bold', fontname='Times New Roman', fontsize=16)
    ...
    position = fig.add_axes([0.92, 0.15, 0.015, .75])  # Colorbar位置
    cb = fig.colorbar(h[3], cax=position)  # 使用最后一个子图显示颜色条
    cb.ax.set_title('Counts', fontdict={"size": 18, "color": "k", 'family': 'Times New Roman'}, pad=10)
    ...
    plt.savefig(resluts_path + 'NEE comparison of all models' + '0316' + '.png', dpi=300)
    # Display the plot
    plt.show()
...
#         # z = (z - np.min(z)) / (np.max(z) - np.min(z))
#
#         # 新增：可选的密度重缩放，使得高密度更加明显
#         z_rescaled = np.log1p(z)  # 使用对数缩放，使高密度区域更明显
#         z_rescaled = (z_rescaled - np.min(z_rescaled)) / (np.max(z_rescaled) - np.min(z_rescaled))
#
#         rmse = np.sqrt(mean_squared_error(x, y))
#         r2 = r2_score(x, y)
#         n = len(x)
...
#
#         # 在绘图逻辑中调用函数
#         scatter = plot_scatter_with_reduced_low_density(x, y, z_rescaled, axs[row, col], threshold=1,reduction_factor=0.5)
#         # 添加 colorbar，这里确保使用 scatter_handle
#
#         # 如何处理其他图形/轴的配置…
#         scatter_handles.append(scatter)  # 更新此部分以引用新函数的输出，如果需要
#
#         # axs[row, col].set_xlim([common_min, common_max])
#         # axs[row, col].set_ylim([common_min, common_max])
...
#
#     resluts_path = 'E:/The North America ecosystem carbon flux/PPT_GRAPHS/Graphs/model estimate results/'
#     plt.savefig(resluts_path + 'GPP comparison of all models' + '_scenario 28_v2' + '.png', dpi=300)
#     # Display the plot
#     plt.show()