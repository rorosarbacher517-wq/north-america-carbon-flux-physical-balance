...
#     # 第一组值:插值与不插值的一起
    ...
    for i in range(8):  # Modify the range to match the 2x3 grid
        ...
        # 绘制散点图，使用 NaN 处理白色点
        scatter = axs[row, col].scatter(x, y, c=z_cleaned, cmap='Spectral_r', s=0.5, edgecolor='none')

        # 另绘制低于阈值的点为白色
        low_density_indices = np.where(z_cleaned < 0.1)[0]
        if low_density_indices.size > 0:
            axs[row, col].scatter(x[low_density_indices], y[low_density_indices], color='white', s=0.5)
    ...
    plt.savefig(resluts_path + 'GPP comparison of all models' + '_scenario 19' + '.png', dpi=300)
    # Display the plot
    plt.show()