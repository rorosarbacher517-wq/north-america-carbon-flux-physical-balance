...
if __name__ == "__main__":
    ...
    np.save(results_variation_path + 'RF_RL_NEE_x_mask_noise_0.01.npy', x_inter_mask_array)
    np.save(results_variation_path + 'RF_RL_NEE_y_qa_noise_0.01.npy', y_flux_qa_array)
    # 将质量标识的波段去除
    ...
    for ik, kth_data in enumerate(cross_validation_data):
        ...
        # 没有soil water
        RFmodel.fit(RF_input_images_train_norm4_clean[:,:9], RF_trainy_input)
        # r_score = RFmodel.score(RF_input_images_test_norm4_clean[:, :9], RF_testy_input)
        # print(r_score)
        y_pred = RFmodel.predict(RF_input_images_test_norm4_clean[:, :9])
    ...
    np.save(results_variation_path + 'RF_RL_NEE_predict_noise_0.01.npy', Y_pred_filtered_array)
    np.save(results_variation_path + 'RF_RL_NEE_true_noise_0.01.npy', Y_test_filtered_array)
    np.save(results_variation_path + 'RF_RL_NEE_site_noise_0.01.npy', RF_test_sites_array)
    # all_y = np.array([Y_test_filtered_array, Y_pred_filtered_array])
    # fig, ax = plt.subplots(figsize=(8,8))
    # # Define the titles for each subgraph
    # # subgraph_titles = ['RECO(gC$\mathregular{m^{-2}}$$\mathregular{d^{-1}}$)']
