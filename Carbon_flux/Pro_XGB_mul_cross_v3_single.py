...
if __name__ == "__main__":
    ...
    np.save(results_variation_path + 'XGB_RLM_NEE_mask_noise_0.01.npy', x_inter_mask_array)
    np.save(results_variation_path + 'XGB_RLM_NEE_y_qa_noise_0.01.npy', y_flux_qa_array)
    # 将质量标识的波段去除
    ...
    # # # # 根据index划分训练集和测试集
    # index_train, index_test, trainx, trainy, testx, testy, train_covariate, test_covariate, train_sites_counts, test_sites_counts, XGB_trainy, XGB_testy = Data_preprocess.get_training_test_com2 \
    #     (x_image_array, y_image_array, covariate_array, proportion=0.8, split_by_site=True)
    # print(trainx.shape, trainy.shape, testx.shape, testy.shape)
    # k倍交叉验证划分数据集
    cross_validation_data = Data_preprocess.get_cross_validation_training_test(input_images_train_noisy, y_image_array,
                                                                               covariate_array)
    ...
    for kth_data in cross_validation_data:
        # index_train, index_test, trainx, trainy, testx, testy, train_covariate, test_covariate, train_sites_counts, test_sites_counts, XGB_trainy, XGB_testy = cross_validation_data
        trainx, trainy, testx, testy, train_covariate, test_covariate, train_sites_counts, test_sites_counts, XGB_trainy, XGB_testy,test_sites_array = (
        ...
        XGB_testy = XGB_testy[:, :, :].reshape(XGB_testy.shape[0], XGB_testy.shape[1], 3)
        XGB_trainy = XGB_trainy[:, :, :].reshape(XGB_trainy.shape[0], XGB_trainy.shape[1], 3)
        ...
        XGB_input_images_train_norm1 = np.mean(input_images_train_norm1.reshape(-1, 3, 3), axis=(-2, -1))
        # reshape
        XGB_input_images_train_norm2 = XGB_input_images_train_norm1.reshape(input_images_train_norm0.shape[0],
        ...
        # XGB_input_images_train_norm3 = np.concatenate(
        #     (XGB_input_images_train_norm2, input_covariate_train_norm0[:, :, 1:5]), axis=2)
        # 采用了era5land气象数据之后，不用再concatenate
        XGB_input_images_train_norm3 = XGB_input_images_train_norm2
        # reshape
        XGB_input_images_train_norm4 = XGB_input_images_train_norm3.reshape(
            XGB_input_images_train_norm3.shape[0] * XGB_input_images_train_norm3.shape[1],
            XGB_input_images_train_norm3.shape[2])
        XGB_input_images_train_norm4.shape
        # 对自变量值为-9999的数据进行处理
        XGB_train_valid_indices = np.where((XGB_input_images_train_norm4 != -9999).all(axis=1))
        XGB_input_images_train_norm4_clean = XGB_input_images_train_norm4[XGB_train_valid_indices]
        XGB_trainy = XGB_trainy.reshape(XGB_trainy.shape[0] * XGB_trainy.shape[1], 3)
        XGB_trainy_clean = XGB_trainy[XGB_train_valid_indices]
        # 对输入影像进行均值平滑处理
        input_images_test_norm1 = process_pixel_values(input_images_test_norm0)
        XGB_input_images_test_norm1 = np.mean(input_images_test_norm1.reshape(-1, 3, 3), axis=(-2, -1))
        # reshape
        XGB_input_images_test_norm2 = XGB_input_images_test_norm1.reshape(input_images_test_norm0.shape[0],
                                                                        input_images_test_norm0.shape[1],
                                                                        input_images_test_norm0.shape[2])
        # 若考虑气象协变量Rg_daily，PotRad_uStar_daily，Tair_f_daily，VPD_f_daily
        # XGB_input_images_test_norm3 = np.concatenate((XGB_input_images_test_norm2, input_covariate_test_norm0[:, :, 1:5]),
        #                                             axis=2)
        XGB_input_images_test_norm3 = XGB_input_images_test_norm2
        # reshape
        XGB_input_images_test_norm4 = XGB_input_images_test_norm3.reshape(
            XGB_input_images_test_norm3.shape[0] * XGB_input_images_test_norm3.shape[1],
            XGB_input_images_test_norm3.shape[2])
        XGB_input_images_test_norm4.shape
        # 对自变量值为-9999的数据进行处理
        XGB_test_valid_indices = np.where((XGB_input_images_test_norm4 != -9999).all(axis=1))
        XGB_input_images_test_norm4_clean = XGB_input_images_test_norm4[XGB_test_valid_indices]
        XGB_testy = XGB_testy.reshape(XGB_testy.shape[0] * XGB_testy.shape[1], 3)
        XGB_testy_clean = XGB_testy[XGB_test_valid_indices]
        ...
        XGB_test_sites_array = test_sites_array_reshape[XGB_test_valid_indices]
        # print(XGB_input_images_train_norm4_clean[:,:9])
        # # XGB模型 _1
        # XGBmodel = xgb.XGBRegressor(n_estimators=180,
        #                                 max_depth=5,
        #                                 n_jobs=-1,
        #                                 random_state=90)
        # XGB模型 _2
        ...
        XGBmodel = xgb.XGBRegressor(n_estimators=100,
                                    n_jobs=-1,
                                    random_state=90)
        # XGBmodel = RandomForestRegressor(n_estimators=180, max_depth=6,
        #                                 min_samples_leaf=6,
        #                                 min_samples_split=7,
        #                                 max_leaf_nodes=41,
        #                                 n_jobs=-1, random_state=90)
        # [0,1,2,3,4,5,6,9,10,11,12,13,14]
        XGB_trainy_input = XGB_trainy_clean[:,0:1].ravel()
        XGB_testy_input = XGB_testy_clean[:,0:1].ravel()
        XGBmodel.fit(XGB_input_images_train_norm4_clean[:,:], XGB_trainy_input)
        # r_score = XGBmodel.score(XGB_input_images_test_norm4_clean[:, :], XGB_testy_input)
        # print(r_score)
        y_pred = XGBmodel.predict(XGB_input_images_test_norm4_clean[:, :])
        y_test = XGB_testy_input
        print(y_pred.shape)
        # 将testy转换为float类型
        Y_test_filtered.append(y_test)
        Y_pred_filtered.append(y_pred)
        XGB_test_sites.append(XGB_test_sites_array)
    Y_pred_filtered_array = np.concatenate(Y_pred_filtered, axis=0)
    Y_test_filtered_array = np.concatenate(Y_test_filtered, axis=0)
    XGB_test_sites_array = np.concatenate(XGB_test_sites, axis=0)
    # 将交叉验证的结果保存下来进行验证分析
    # results_variation_path = 'E:/Paper code/code/Vegetation_productivity_prediction/Sites_estimation/data/Sites_estimate_result/'
    np.save(results_variation_path + 'XGB_RLM_NEE_predict_noise_0.01.npy', Y_pred_filtered_array)
    np.save(results_variation_path + 'XGB_RLM_NEE_true_noise_0.01.npy', Y_test_filtered_array)
    np.save(results_variation_path + 'XGB_RLM_NEE_site_noise_0.01.npy', XGB_test_sites_array)
    ...
    # # plot_title = 'Density Plots of ' + 'using reflectance,lai/fpar and meteorology by XGB model'
    # # plot_title = 'Density Plots of ' + 'using reflectance and meteorology by XGB model'
    # # plot_title = 'Density Plots of ' + 'using reflectance and lai/fpar variables by XGB model'
    # # plot_title = 'Density Plots of ' + 'using reflectance variables by XGB model'
