...
#     # 第一组值:插值与不插值的一起
    ...
    for i in range(8):
        ...
        # Define custom colors and bounds
        colors = [
            'white', 'lightcyan', 'lightblue', 'deepskyblue', 'blue', 'mediumseagreen',
            'green', 'greenyellow', 'yellow', 'gold', 'orange', 'tomato', 'red', 'firebrick', 'darkred'
        ]
        n_bins = len(colors)
        max_density = hist.max()
        bounds = np.logspace(np.log10(1), np.log10(max_density + 1), n_bins + 1)
        filtered_colors = [color if bound > density_threshold else 'white' for color, bound in
                           zip(colors, bounds[:n_bins])]
        cmap = ListedColormap(filtered_colors)
        norm = BoundaryNorm(bounds, cmap.N)

        # Plot histogram with custom color mapping
        h = axs[row, col].hist2d(x, y, bins=bins_n, cmap=cmap, norm=norm)
        # confidence_ellipse(x, y, axs[row, col], edgecolor='red')
        # Display RMSE and CC on the plot
        text = f"RMSE = {rmse:.2f}\nCC = {cc:.2f}"
        ...
        cb = plt.colorbar(h[3], ax=axs[row, col], orientation='vertical', pad=0.02)
        # cb.set_label('Density', labelpad=10, fontsize=16, fontname='Times New Roman')
        cb.ax.xaxis.set_label_position('top')  # Move label to the top
    ...
    plt.savefig(resluts_path + 'NEE comparison of all models' + '_scenario 0213' + '.png', dpi=300)
    # Display the plot
    plt.show()
