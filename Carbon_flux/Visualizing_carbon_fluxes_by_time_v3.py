# 统计每个模型 不同时间内 对应的NEE GPP RECO  只显示日尺度和月尺度 以及'XGBoost',# 'RF', # 'Physics-aware transformer',
import pandas as pd
import numpy as np
...
#     # RF 有气象变量
    ...
    def compute_annual_CC_and_plot(X_values, Y_pred_values, y_variables, annual_data, ax, title):
        ...
        for year in unique_years:
            ...
            if np.sum(mask) == 0:
                continue
            for i, var in enumerate(y_variables[:3]):
                if Y_pred_values[var].shape != X_values.shape:
                    raise ValueError(f"Shape mismatch for {var}: {Y_pred_values[var].shape} vs {X_values.shape}")
                CC = np.corrcoef(X_values[mask], Y_pred_values[var][mask])[0, 1]
                # CC = np.sqrt(np.mean((Y_pred_values[var][mask] - X_values[mask]) ** 2))
    ...
    fig.legend(handles=handles, loc='lower right',ncol=len(model_names), fontsize=16, columnspacing=1.4, markerscale=2)
    # # 设置图例，`bbox_to_anchor`用于确定图例位置，`ncol`用于定义列数
    ...
    # 保存与显示
    plt.savefig(results_path + 'Combined CC of five models 0419 without meteo.png', dpi=300, bbox_inches='tight')
    plt.show()
