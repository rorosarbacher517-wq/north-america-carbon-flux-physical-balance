...
if __name__ == "__main__":
    ...
    inter_mask = np.load(inputdata_path + 'DL_RL_x_mask_no_physical_3.npy', allow_pickle=True)
    DL_y_qa = np.load(inputdata_path + 'DL_RL_y_qa_no_physical_3.npy', allow_pickle=True)
    # DL_Y_true[:,:,1]
...
#     # 第一组值:插值与不插值的一起
    ...
    # X_variables = ['Ground_NEE_0_RLM', 'Ground_GPP_0_RLM', 'Ground_NEE_0_RLM', 'Ground_NEE_0', 'Ground_GPP_0', 'Ground_NEE_0']
    # Y_variables = ['DL_NEE_0_no_physical_RLM', 'DL_GPP_0_no_physical_RLM', 'DL_NEE_0_no_physical_RLM', 'DL_NEE_0_no_physical', 'DL_GPP_0_no_physical', 'DL_NEE_0_no_physical']
    ...
    # model_subtitle = ['Time series deep learning model\nwith physical constraint','Time series deep learning model\nwithout physical constraint','XGBoost', 'RF'] # 'CFR without physical constraints',
    model_subtitle = ['Physics-aware transformer','Transformer', 'XGBoost', 'RF']
    ...
    # # 首先收集所有密度大于0.1的点的数据
    # for i in range(8):  # Adjust the range to match the grid size
    ...
    #
    #     # 计算密度值
    #     z = gaussian_kde(xy, bw_method=partial(my_kde_bandwidth, fac=0.5))(xy)
    #
    #     # Normalize density
    ...
    #
    # # 计算公共的最小值和最大值
    # common_x_min = np.min(all_x)
    # common_x_max = np.max(all_x)
    # common_y_min = np.min(all_y)
    # common_y_max = np.max(all_y)
    #
    # # 设置公共的最小值和最大值
    # common_min = min(common_x_min, common_y_min)
    # common_max = max(common_x_max, common_y_max)
    ...
    for i in range(8):  # Adjust the range to match the grid size
        ...
        x_variable = X_variables[i]
        y_variable = Y_variables[i]
        x = globals()[x_variable] # Original x data
        y = globals()[y_variable] # Original y data
        ...
        # # Apply the density threshold
        mask = z >= 0.1
        xx = x[mask]  # Filtered x based on density
        yy = y[mask]  # Filtered y based on density
        zz = z[mask]  # Filtered z based on density
        ...
        axs[row, col].set_aspect('equal', adjustable='box')  # 1:1 aspect ratio
        ...
        # Scatter plot after applying the density threshold
        scatter = axs[row, col].scatter(xx, yy, c=zz, cmap='Spectral_r', s=10)
        ...
        print(f'Computing metrics for={i}: RMSE={rmse}, CC={cc}, Bias={bias}')
    ...
    plt.subplots_adjust(left=0.1, right=0.9, bottom=0.15, top=0.9, wspace=0.3, hspace=0.3)

    # Save results
    results_path = 'E:/The North America ecosystem carbon flux/PPT_GRAPHS/Graphs/model estimate results/'
    plt.savefig(results_path + 'GPP comparison of all models' + '_scenario 16' + '.png', dpi=300)
