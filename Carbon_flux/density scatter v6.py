...
#     # 第一组值:插值与不插值的一起
    ...
    for i in range(8):  # Modify the range to match the 2x3 grid
        ...
        # 绘制散点图，调整低密度点的透明度
        scatter = axs[row, col].scatter(x, y, c=z, cmap='Spectral_r', s=5, edgecolor='none',
                                        alpha=np.where(z < 0.1, 0.2, 1.0))

        # 不再单独绘制低密度点，保持它们的颜色，只调整透明度
    ...
    plt.savefig(resluts_path + 'GPP comparison of all models' + '_scenario 26' + '.png', dpi=300)
    # Display the plot
    plt.show()