...
#     # 第一组值:插值与不插值的一起
    ...
    for i in range(8):  # Modify the range to match the 2x3 grid
        ...
        # 在绘图逻辑中调用函数
        scatter = plot_scatter_with_reduced_low_density(x, y, z, axs[row, col], threshold=0.15,reduction_factor=0.5)
    ...
    plt.savefig(resluts_path + 'GPP comparison of all models' + '_scenario 27_v1' + '.png', dpi=300)
    # Display the plot
    plt.show()