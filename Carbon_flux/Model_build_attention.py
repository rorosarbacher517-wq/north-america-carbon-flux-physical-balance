...
def get_transformer_transformer(layer_cnn=2, layern=3, units=64, n_times=80,
    ...
    coords_h = np.arange(n_window);
    coords_w = np.arange(n_window)
    ...
    for ti in range(n_times):
        ...
        attn_output, attn4 = atten_layer(query=x3, value=x3, key=x3, return_attention_scores=True,
                                         attention_mask=x3_mask[:, tf.newaxis, tf.newaxis, :])
        if drop > 0:
...
def transformer_block(queryx, x, padding_mask, units, n_head, reg=None, drop=0.1, is_batch=True, is_att_score=True):
    if is_att_score:
        attn_output, attn4 = MultiHeadAttention(key_dim=units // n_head, num_heads=n_head, kernel_regularizer=reg)(
            query=queryx, value=x, key=x,
            return_attention_scores=is_att_score, attention_mask=padding_mask)
    else:
        attn_output = MultiHeadAttention(key_dim=units // n_head, num_heads=n_head, kernel_regularizer=reg)(
            query=queryx, value=x, key=x,
            return_attention_scores=is_att_score, attention_mask=padding_mask)
...
def get_transformer_cnn3(layer_cnn=1, layern=2, units=64, n_times=80,
    ...
    # x_modis = input_images_train_norm3[:, :, :, :, :MODIS_BANDS_N]
    ...
    for i in range(layer_cnn):
        x_input_modis = transformer_block(x_input_modis, x_input_modis, mask_modis, units, n_head, drop=drop,
                                          is_batch=True, is_att_score=False);
        if is_rptv:
            x_input_meter = transformer_block(x_input_meter, x_input_meter, mask_meter, units, n_head, drop=drop,
                                              is_batch=True, is_att_score=False);
    ...
    attention_all = []
    for i in range(layern):
        x,attention = transformer_block(x, x, mask_modis, units, n_head, drop=drop, is_batch=True, is_att_score=True);
        attention_all.append(attention)
    # method 1 simple linear
    # output = Dense(n_out, activation="linear")(enc_output)
    attention_all_tensor = tf.stack(attention_all, axis=0)
    ## method 2
    enc_output = x
    for ii in range(n_out):
        if ii == 0:
    ...
    model = Model(inputs=inputs, outputs={'predictions': output, 'attentions': attention_all_tensor})
    return model
