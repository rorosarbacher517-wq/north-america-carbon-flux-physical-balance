...
if __name__ == "__main__":
    ...
    for ik, kth_data in enumerate(cross_validation_data):
        ...
        model_history = Model_history_attention.my_train_1schedule_time_drop(
            model, input_images_train_norm3, trainy,
        ...
        model_pre_name = f'./region model/get_cnn_transformer_RLM_0420_{ik}.h5'
        ...
        # 测试集预测
        input_images_test_norm3 = np.array(input_images_test_norm3, dtype=np.float32)
        input_images_test_norm3 = tf.convert_to_tensor(input_images_test_norm3)

        test_batch_size = 32
        test_n = input_images_test_norm3.shape[0]
        predictions_list, attentions_list = [], []

        for i in range(0, test_n, test_batch_size):
            batch_x = input_images_test_norm3[i:i + test_batch_size]
        ...
        Y_pred_filtered.append(predictions)
        Y_test_sites.append(test_sites_array)
        Y_attention.append(attentions)
    ...
    Y_attention_array = np.concatenate(Y_attention, axis=1)
    ...
    np.save(results_variation_path + 'DL_RLM_attention.npy', Y_attention_array)