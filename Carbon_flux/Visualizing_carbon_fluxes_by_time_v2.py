...
#     # RF 有气象变量
    ...
    # 统计每天的相关系数并绘图
    def compute_daily_avg_CC_and_plot(X_values, Y_pred_values, y_variables, doy_data, ax, title):
    ...
    # 统计每月的相关系数并绘图
    # 统计每月的相关系数并绘图
    def compute_monthly_avg_CC_and_plot(X_values, Y_pred_values, y_variables, month_data, ax, title):
        # 将 month_data 展平成一维数组并去除无效值
        ...
        # 遍历每个 DOY，按月份聚合数据
        for i, month in enumerate(flat_MONTH):
            month_days = days_in_month[month]

            if month not in monthly_aggregated_values:
                monthly_aggregated_values[month] = {'X': [], 'Y': []}

            # 加权
            for var in y_variables[:5]:  # 只处理前5个模型
                monthly_aggregated_values[month]['X'].append(X_values[i] * month_days)  # X 值乘以月天数
                monthly_aggregated_values[month]['Y'].append(Y_pred_values[var][i] * month_days)  # Y 值乘以月天数

        # 计算每个月的加权相关系数
        for month, values in monthly_aggregated_values.items():
            X_vals = np.array(values['X'])
            Y_vals = np.array(values['Y']).flatten()

            # 确保 Y_vals 是二维的，若不是则调整
            if Y_vals.ndim == 1:
                Y_vals = Y_vals.reshape(-1, 1)  # 将一维转为二维

            for i, var in enumerate(y_variables[:5]):
                CC = np.corrcoef(X_vals.flatten(), Y_vals[:,0].flatten())[0, 1]  # 计算每个模型的 CC
                CC_results.append({'Month': month, 'Model': model_names[i], 'CC': CC})

        # 计算每个月的平均 CC 值
        df_CC = pd.DataFrame(CC_results)
        avg_CC = df_CC.groupby(['Month', 'Model'])['CC'].mean().reset_index()

        # 绘制月度相关系数图
        palette = [color_mapping[model] for model in model_names]
        sns.lineplot(data=avg_CC, x='Month', y='CC', hue='Model', marker='o',
                     palette=palette, ax=ax, markersize=4, legend=False, markeredgecolor='none')

        ax.set_title(title, fontname='Arial', fontsize=18)
        ax.set_ylabel('Average CC', fontname='Arial', fontsize=18)
        ax.set_xticks(np.arange(12))  # 设置 x 轴为 0 到 11 月份
        ax.set_xticklabels(np.arange(1, 13), rotation=45, ha='right')  # 标签为 1 到 12 月
        ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{x:.2f}'))
    ...
    # 统计年度的相关系数并绘图
    def compute_annual_avg_CC_and_plot(X_values, Y_pred_values, y_variables, doy_data, ax, title):
        # 将 doy_data 展平成一维数组并去除无效值
        flattened_DY = doy_data.flatten()
        valid_indices = flattened_DY != -9999
        flat_DY = flattened_DY[valid_indices]
        ...
        # 创建一个空的字典来保存每年汇总的数据
        annual_aggregated_values = {}

        # 按年份进行数据汇总
        for i, doy in enumerate(flat_DY):
            year = (doy - 1) // 365 + 1  # 假设年份从1开始

            if year not in annual_aggregated_values:
                annual_aggregated_values[year] = {'X': [], 'Y': []}

            # 加权，乘以 365 天
            for var in y_variables[:5]:
                annual_aggregated_values[year]['X'].append(X_values[i])  # 直接使用 X 值
                annual_aggregated_values[year]['Y'].append(Y_pred_values[var][i])  # 直接使用 Y 值

        # 计算每年的加权相关系数
        for year, values in annual_aggregated_values.items():
            X_vals = np.array(values['X'])
            Y_vals = np.array(values['Y'])

            for i, var in enumerate(y_variables[:5]):
                CC = np.corrcoef(X_vals.flatten(), Y_vals[:, i].flatten())[0, 1]  # 计算每个模型的 CC
                CC_results.append({'Year': year, 'Model': model_names[i], 'CC': CC})

        # 计算每个年份的平均 CC 值
        df_CC = pd.DataFrame(CC_results)
        avg_CC = df_CC.groupby(['Year', 'Model'])['CC'].mean().reset_index()

        # 绘制年度相关系数图
        palette = [color_mapping[model] for model in model_names]
        sns.lineplot(data=avg_CC, x='Year', y='CC', hue='Model', marker='o',
                     palette=palette, ax=ax, markersize=4, legend=False, markeredgecolor='none')

        ax.set_title(title, fontname='Arial', fontsize=18)
        ax.set_ylabel('Average CC', fontname='Arial', fontsize=18)
        ax.set_xticks(np.unique(list(annual_aggregated_values.keys()))[::2])  # 每隔两个年份显示一次
        ax.set_xticklabels(np.unique(list(annual_aggregated_values.keys()))[::2], rotation=45, ha='right')
        ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{x:.2f}'))
    ...
    for i, (key, value) in enumerate(variables.items()):
        ...
        # 计算并绘制月度 CC
        compute_monthly_avg_CC_and_plot(
            value['X_values'],
            value['Y_pred_values'],
            value['Y_variables'],
            value['Month_data'],
        ...
        # 计算并绘制年度 CC
        compute_annual_avg_CC_and_plot(
            value['X_values'],
            value['Y_pred_values'],
            value['Y_variables'],
            value['Annual_data'],
    ...
    # # 保存与显示
    # plt.savefig(results_path + 'Combined CC of all models 0405.png', dpi=300, bbox_inches='tight')
    plt.show()
