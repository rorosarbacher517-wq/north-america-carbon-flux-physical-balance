...
MODIS_BANDS_N = 9
def my_train_1schedule_time_drop(model,train_datasetx,train_datasety,epochs=10,start_rate=0.001,loss=np.nan,per_epoch=100,split_epoch=4,option=0,decay=1e-5, 
        batch_size=32,validation_split=0.04,hold_epoch=0,reduce_epoch=False, gaps_p=0.1, gaps_n=0):
    ...
    def random_mask(x1,y1):
        ...
        input_length = tf.math.reduce_sum(tf.cast(x_mask,tf.int32))  # !!!!!!!!!!!!! # a bug fixed on Mar 4, 2013 !!!!!!!!!!!!!!!!!!!!!!
        ...
        # if bool(delete_n>0):
            ...
            uniform_distribution = tf.random.uniform(shape=[input_length], minval=0, maxval=None, dtype=tf.float32, seed=None, name=None)
            ...
            t9 = tf.SparseTensor(indices=tf.reshape(deleted_y,[delete_n,1]), values=updates,  dense_shape=[x1.shape[0]])
    ...
    if validation_split==0:
        ...
        X_train_sub, y_train_sub, X_validation, y_validation, training_index, validation_index = \
            train_test.random_split_train_validation (X_train=train_datasetx,y_train=train_datasety,pecentage = validation_split)    
    ...
    with tf.device("/cpu:0"): ## this is added on Aug 21, 2021 to handle tensorflow 2.6 by forcing the prefetch on CPU
        ...
        train_ds = tf.data.Dataset.from_tensor_slices( (X_train_sub, y_train_sub)).shuffle(train_n+1).map(lambda x, y: random_mask(x,y)).batch(batch_size)
    ...
    if option==-1:
        print ('tfa.optimizers.SGD '); optimizer=tf.keras.optimizers.SGD(learning_rate=schedule_one,momentum=momentum); 
    elif option==0:
    ...
    import socket
    if validation_split!=0:
        model_history = model.fit(train_ds, validation_data=validation_ds, epochs=final_epochs, verbose=2, callbacks=[LRTensorBoard(log_dir="./tmp/tb_log/"+socket.gethostname() )])
    else:
        model_history = model.fit(train_ds, epochs=final_epochs, verbose=2, callbacks=[LRTensorBoard(log_dir="./tmp/tb_log/"+socket.gethostname() )])
...
def my_train_1schedule(model, input_images_train_norm3, train_datasety, epochs=10, start_rate=0.001, loss=np.nan, per_epoch=100,
    ...
    if validation_split > 0:
        ...
        model_history = model.fit(x=train_datasetx, y = train_datasety, validation_split=validation_split, epochs=final_epochs,
