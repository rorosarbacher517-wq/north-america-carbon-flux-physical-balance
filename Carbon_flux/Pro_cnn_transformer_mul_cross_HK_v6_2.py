...
if __name__ == "__main__":
    ...
    np.save(results_variation_path + 'DL_RLM_x_mask_soil_1.npy', x_inter_mask_array)
    np.save(results_variation_path + 'DL_RLM_y_qa_soil_1.npy',
            y_flux_qa_array)  # y_flux_qa_array.shape (1688, 365, 5), the fouth dimension is QA
    # 将质量标识的波段去除
    ...
    for ik, kth_data in enumerate(cross_validation_data):
        ...
        input_images_train_norm0 = other_process.process_pixel_values(input_images_train_norm0[:,:,:11,:,:])
        input_images_test_norm0 = other_process.process_pixel_values(input_images_test_norm0[:,:,:11,:,:])
        # 复制数据 变换维度
    ...
    np.save(results_variation_path + 'DL_RLM_predict_soil_1.npy', Y_pred_filtered_array)
    np.save(results_variation_path + 'DL_RLM_true_soil_1.npy', Y_test_filtered_array)
    np.save(results_variation_path + 'DL_RLM_sites_soil_1.npy', Y_test_sites_array)
