# 用三组数据（all  modis——high quality; Y_quality）分别作为输入，来比较数据敏感性
...
if __name__ == "__main__":
    ...
    x_image_array, y_image_array, x_inter_mask_array0, y_qa_array = Data_filter_match.mul_match_x_y_modis_quality(
        ...
        NEE_GPP_RECO_filledvalue=0,is_inter=2)
    ...
    np.save(results_variation_path + 'DL_RLM_x_mask_good_y.npy', x_inter_mask_array)
    np.save(results_variation_path + 'DL_RLM_y_qa_good_y.npy',y_flux_qa_array)  # y_flux_qa_array.shape (1688, 365, 5), the fouth dimension is QA
    # 将质量标识的波段去除
    ...
    # np.save(results_variation_path + 'DL_RL_predict_good_y.npy', Y_pred_filtered_array)
    # np.save(results_variation_path + 'DL_RL_true_good_y.npy', Y_test_filtered_array)
    # np.save(results_variation_path + 'DL_RL_sites_good_y.npy', Y_test_sites_array)