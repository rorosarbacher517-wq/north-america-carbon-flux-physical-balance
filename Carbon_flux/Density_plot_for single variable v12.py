...
if __name__ == "__main__":
    ...
    DL_Y_predict_RL_noise = np.load(inputdata_path + 'DL_RL_predict_noise.npy', allow_pickle=True)
    DL_Y_true_RL_noise = np.load(inputdata_path + 'DL_RL_true_noise.npy', allow_pickle=True)
    DL_All_true_RL_noise = np.load(inputdata_path + 'DL_RL_sites_noise.npy', allow_pickle=True)
    inter_mask_RL_noise = np.load(inputdata_path + 'DL_RL_x_mask_noise.npy', allow_pickle=True)
    DL_y_qa_RL_noise = np.load(inputdata_path + 'DL_RL_y_qa_noise.npy', allow_pickle=True)
    ...
    DL_Y_predict_RLM_noise = np.load(inputdata_path + 'DL_RLM_predict_noise.npy', allow_pickle=True)
    DL_Y_true_RLM_noise = np.load(inputdata_path + 'DL_RLM_true_noise.npy', allow_pickle=True)
    DL_All_true_RLM_noise = np.load(inputdata_path + 'DL_RLM_sites_noise.npy', allow_pickle=True)
    inter_mask_RLM_noise = np.load(inputdata_path + 'DL_RLM_x_mask_noise.npy', allow_pickle=True)
    DL_y_qa_RLM_0noise = np.load(inputdata_path + 'DL_RLM_y_qa_noise.npy', allow_pickle=True)
    ...
    DL_NEE_noise_RLM  = DL_Y_predict_RLM_noise[:,:,0]

    Ground_NEE_0_RLM = DL_Y_true_RLM_0[:, :, 0]
    Ground_NEE_noise_RLM = DL_Y_true_RLM_noise[:, :, 0]

    # DL RLM单独预测的碳通量 没有气象变量
    # 第一组值:插值与不插值的一起
    # **Step 1: 创建掩码（值不等于 -9999）**
    mask_0 = Ground_NEE_0_RLM != -9999
    mask_1 = Ground_NEE_noise_RLM != -9999

    DL_NEE_0_RLM_filtered = DL_NEE_0_RLM[mask_0]
    DL_NEE_noise_RLM_filtered = DL_NEE_noise_RLM[mask_1]

    Ground_NEE_0_RLM_filtered = Ground_NEE_0_RLM[mask_0]
    Ground_NEE_noise_RLM_filtered = Ground_NEE_noise_RLM[mask_1]

    # 没有气象数据的
    DL_NEE_0_RL = DL_Y_predict_RL_0[:, :, 0]
    DL_NEE_noise_RL = DL_Y_predict_RL_noise[:, :, 0]

    Ground_NEE_0_RL = DL_Y_true_RL_0[:, :, 0]
    Ground_NEE_noise_RL = DL_Y_true_RL_noise[:, :, 0]

    # DL RLM单独预测的碳通量 没有气象变量
    # 第一组值:插值与不插值的一起
    # **Step 1: 创建掩码（值不等于 -9999）**
    mask_RL_0 = Ground_NEE_0_RL != -9999
    mask_RL_noise = Ground_NEE_noise_RL != -9999

    DL_NEE_0_RL_filtered = DL_NEE_0_RL[mask_RL_0]
    DL_NEE_noise_RL_filtered = DL_NEE_noise_RL[mask_RL_noise]

    Ground_NEE_0_RL_filtered = Ground_NEE_0_RL[mask_RL_0]
    Ground_NEE_noise_RL_filtered = Ground_NEE_noise_RL[mask_RL_noise]
################## gpp
    DL_GPP_0_RLM = DL_Y_predict_RLM_0[:, :, 1]
    DL_GPP_noise_RLM = DL_Y_predict_RLM_noise[:, :, 1]

    Ground_GPP_0_RLM = DL_Y_true_RLM_0[:, :, 1]
    Ground_GPP_noise_RLM = DL_Y_true_RLM_noise[:, :, 1]

    # DL RLM单独预测的碳通量 没有气象变量
    # 第一组值:插值与不插值的一起
    # **Step 1: 创建掩码（值不等于 -9999）**
    mask_GPP_RLM_0 = Ground_GPP_0_RLM != -9999
    mask_GPP_RLM_noise = Ground_GPP_noise_RLM != -9999

    DL_GPP_0_RLM_filtered = DL_GPP_0_RLM[mask_GPP_RLM_0]
    DL_GPP_noise_RLM_filtered = DL_GPP_noise_RLM[mask_GPP_RLM_noise]

    Ground_GPP_0_RLM_filtered = Ground_GPP_0_RLM[mask_GPP_RLM_0]
    Ground_GPP_noise_RLM_filtered = Ground_GPP_noise_RLM[mask_GPP_RLM_noise]

    # 没有气象数据的
    DL_GPP_0_RL = DL_Y_predict_RL_0[:, :, 1]
    DL_GPP_noise_RL = DL_Y_predict_RL_noise[:, :, 1]

    Ground_GPP_0_RL = DL_Y_true_RL_0[:, :, 1]
    Ground_GPP_noise_RL = DL_Y_true_RL_noise[:, :, 1]

    # DL RLM单独预测的碳通量 没有气象变量
    # 第一组值:插值与不插值的一起
    # **Step 1: 创建掩码（值不等于 -9999）**
    mask_GPP_RL_0 = Ground_GPP_0_RL != -9999
    mask_GPP_RL_noise = Ground_GPP_noise_RL != -9999

    DL_GPP_0_RL_filtered = DL_GPP_0_RL[mask_GPP_RL_0]
    DL_GPP_noise_RL_filtered = DL_GPP_noise_RL[mask_GPP_RL_noise]

    Ground_GPP_0_RL_filtered = Ground_GPP_0_RL[mask_GPP_RL_0]
    Ground_GPP_noise_RL_filtered = Ground_GPP_noise_RL[mask_GPP_RL_noise]
    ################RECO
    DL_RECO_0_RLM = DL_Y_predict_RLM_0[:, :, 2]
    DL_RECO_noise_RLM = DL_Y_predict_RLM_noise[:, :, 2]

    Ground_RECO_0_RLM = DL_Y_true_RLM_0[:, :, 2]
    Ground_RECO_noise_RLM = DL_Y_true_RLM_noise[:, :, 2]

    # DL RLM单独预测的碳通量 没有气象变量
    # 第一组值:插值与不插值的一起
    # **Step 1: 创建掩码（值不等于 -9999）**
    mask_RECO_RLM_0 = Ground_RECO_0_RLM != -9999
    mask_RECO_RLM_noise = Ground_RECO_noise_RLM != -9999

    DL_RECO_0_RLM_filtered = DL_RECO_0_RLM[mask_RECO_RLM_0]
    DL_RECO_noise_RLM_filtered = DL_RECO_noise_RLM[mask_RECO_RLM_noise]

    Ground_RECO_0_RLM_filtered = Ground_RECO_0_RLM[mask_RECO_RLM_0]
    Ground_RECO_noise_RLM_filtered = Ground_RECO_noise_RLM[mask_RECO_RLM_noise]

    # 没有气象数据的
    DL_RECO_0_RL = DL_Y_predict_RL_0[:, :, 2]
    DL_RECO_noise_RL = DL_Y_predict_RL_noise[:, :, 2]

    Ground_RECO_0_RL = DL_Y_true_RL_0[:, :, 2]
    Ground_RECO_noise_RL = DL_Y_true_RL_noise[:, :, 2]

    # DL RLM单独预测的碳通量 没有气象变量
    # 第一组值:插值与不插值的一起
    # **Step 1: 创建掩码（值不等于 -9999）**
    mask_RECO_RL_0 = Ground_RECO_0_RL != -9999
    mask_RECO_RL_noise = Ground_RECO_noise_RL != -9999

    DL_RECO_0_RL_filtered = DL_RECO_0_RL[mask_RECO_RL_0]
    DL_RECO_noise_RL_filtered = DL_RECO_noise_RL[mask_RECO_RL_noise]

    Ground_RECO_0_RL_filtered = Ground_RECO_0_RL[mask_RECO_RL_0]
    Ground_RECO_noise_RL_filtered = Ground_RECO_noise_RL[mask_RECO_RL_noise]
    ...
    X_variables = ['Ground_NEE_0_RLM_filtered','Ground_GPP_0_RLM_filtered','Ground_RECO_0_RLM_filtered',
                   'Ground_NEE_noise_RLM_filtered','Ground_GPP_noise_RLM_filtered','Ground_RECO_noise_RLM_filtered']
    Y_variables = ['DL_NEE_0_RLM_filtered','DL_GPP_0_RLM_filtered','DL_RECO_0_RLM_filtered',
                   'DL_NEE_noise_RLM_filtered','DL_GPP_noise_RLM_filtered','DL_RECO_noise_RLM_filtered']
    # X_variables = ['Ground_NEE_0_RL_filtered', 'Ground_GPP_0_RL_filtered', 'Ground_RECO_0_RL_filtered',
    #                'Ground_NEE_noise_RL_filtered', 'Ground_GPP_noise_RL_filtered',
    #                'Ground_RECO_noise_RL_filtered']
    # Y_variables = ['DL_NEE_0_RL_filtered', 'DL_GPP_0_RL_filtered', 'DL_RECO_0_RL_filtered',
    #                'DL_NEE_noise_RL_filtered', 'DL_GPP_noise_RL_filtered',
    #                'DL_RECO_noise_RL_filtered']
    ...
    plt.savefig(resluts_path + 'Model with noise_RLM_0306' + '.png', dpi=300)
    # Display the plot
    plt.show()
