...
import mpl_scatter_density   # 导入mpl_scatter_density库
...
#     # 第一组值:插值与不插值的一起
    ...
    fig, axs = plt.subplots(2, 4, figsize=(16, 9), subplot_kw={'projection': 'scatter_density'})
    # fig.subplots_adjust(hspace=0.4, wspace=0.4)

    for i in range(8):  # Loop to fit a 2x4 grid
        row = i // 4
        col = i % 4
        ...
        density = axs[row, col].scatter_density(x, y,norm=norm, cmap='Spectral_r', vmin=0.1, vmax=1.0)
    ...
    cbar_ax = fig.add_axes([0.92, 0.15, 0.015, .75])
    cb = fig.colorbar(density, cax=cbar_ax, label='Density')
    cb.ax.set_title('Density', fontdict={'size': 18, 'color': 'k', 'family': 'Times New Roman'}, pad=10)
    ...
    # plt.savefig(resluts_path + 'RECO comparison of all models' + '_scenario 29_v5' + '.png', dpi=300)
    # Display the plot
    plt.show()
