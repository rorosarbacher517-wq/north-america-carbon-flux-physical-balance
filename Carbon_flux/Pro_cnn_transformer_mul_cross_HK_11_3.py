...
if __name__ == "__main__":
    ...
    #打印x_image_array[:,:,:,:,:] site-year的最长有效长度和最短有效长度  有效长度是指一年中不为-9999的长度
    ...
    np.save(results_variation_path + 'DL_RLM_x_image_array_attention.npy', x_image_array)
    np.save(results_variation_path + 'DL_RLM_y_image_array_attention.npy', y_image_array)
    np.save(results_variation_path + 'DL_RLM_covariate_array_attention.npy', covariate_array)
    # # k倍交叉验证划分数据集
    ...
    for ik, kth_data in enumerate(cross_validation_data):
        ...
        # Build the model
        model, attention_scores_1 = Model_build.get_transformer_cnn3(layer_cnn=0, layern=3, units=units,
                                                                     n_times=input_images_train_norm3.shape[1],
                                                                     n_window=3, n_head=4, drop=drop, n_out=3,
                                                                     is_batch=True,
                                                                     mask_value=-9999.0, is_rptv=True, is_sensor=False)
        if ik == 0:
            print(model.summary())  # Output model summary
        ...
        model_pre_name = f'/region model/get_cnn_transformer_attention_{ik + 1}.h5'
        model.save(model_pre_name)  # Save the trained model
        ...
        # Create a new model for prediction that outputs both predictions and attention scores
        attention_model = tf.keras.models.Model(inputs=model.inputs, outputs=[model.output] + model.layers[
            -1].output)  # Corrected to use the actual attention layer outputs

        # Predict and retrieve attention scores
        predictions = attention_model.predict(input_images_test_norm3, verbose=2)
        y_pred_filtered = predictions[0] / y_SCALE  # Scale predictions
        attention_scores = predictions[1:]  # Store attention scores

        # Convert testy to float
        ...
        Y_attention_scores.append(attention_scores)  # Append attention scores
    ...
    Y_attention_scores_array = np.concatenate(Y_attention_scores, axis=0)

    # Save the results
    np.save(results_variation_path + 'DL_RLM_predict_attention.npy', Y_pred_filtered_array)
    np.save(results_variation_path + 'DL_RLM_true_attention.npy', Y_test_filtered_array)
    np.save(results_variation_path + 'DL_RLM_sites_attention.npy', Y_test_sites_array)
    np.save(results_variation_path + 'DL_RLM_attention_scores.npy', Y_attention_scores_array)
