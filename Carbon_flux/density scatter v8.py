...
#     # 第一组值:插值与不插值的一起
    ...
    for i in range(8):  # Modify the range to match the 2x3 grid
        ...
        # 使用幂次缩放，调整高低密度的显示
        scaling_factor = 0.5  # 调整因子，<1将增加高密度的比例
        z_scaled = z ** scaling_factor

        # 对密度进行排序，并根据缩放后的z重排x, y
        idx = z_scaled.argsort()
        x, y, z_scaled = x[idx], y[idx], z_scaled[idx]

        # 归一化处理
        z_scaled = (z_scaled - np.min(z_scaled)) / (np.max(z_scaled) - np.min(z_scaled))
        ...
        # 在绘图逻辑中调用函数
        scatter = plot_scatter_with_reduced_low_density(x, y, z_scaled, axs[row, col], threshold=1,reduction_factor=0)
    ...
    plt.savefig(resluts_path + 'RECO comparison of all models' + '_without water v1' + '.png', dpi=300)
    # Display the plot
    plt.show()
