# 将深度学习单独预测的加进来 深度学习三个加物理约束 深度学习三个不加约束 深度学习单个 XGBoost RF
import pandas as pd
import numpy as np
...
if __name__ == "__main__":
    ...
    DL_Y_predict_RLM = np.load("D:\Carbon_flux\data\sites_estimate_result\DL_RLM_predict5.npy", allow_pickle=True)
    DL_Y_true_RLM = np.load("D:\Carbon_flux\data\sites_estimate_result\DL_RLM_true5.npy", allow_pickle=True)
    DL_All_true_RLM = np.load("D:\Carbon_flux\data\sites_estimate_result\DL_RLM_sites5.npy", allow_pickle=True)
    inter_mask_RLM = np.load("D:\Carbon_flux\data\sites_estimate_result\DL_RLM_x_mask5.npy", allow_pickle=True)
    DL_y_qa_RLM= np.load("D:\Carbon_flux\data\sites_estimate_result\DL_RLM_y_qa5.npy", allow_pickle=True)
...
#     # RF 有气象变量
    ...
    X_variables = ['Ground_RECO_0_RLM','Ground_RECO_0_RLM','Ground_RECO_0_RLM','Ground_RECO_0_RLM','Ground_RECO_0_RLM','Ground_RECO_0_RLM','Ground_RECO_0','Ground_RECO_0','Ground_RECO_0', 'Ground_RECO_0']
    Y_variables = ['DL_RECO_0_RLM','DL_RECO_0_no_physical_RLM', 'DL_RECO_0_RLM_Single','XGB_RECO_0_RLM', 'RF_RECO_0_RLM','DL_RECO_0','DL_RECO_0_no_physical','DL_RECO_0_RL_Single','XGB_RECO_0', 'RF_RECO_0']
    #*************************************************
    # GPP 与 RECO-GPP的最大最小中位数等
    bias_transformer_single_variable_RL = np.abs(RF_NEE_0_RLM-(RF_RECO_0_RLM-RF_GPP_0_RLM))

    # # 计算统计量
    # minimum = np.min(bias_transformer_single_variable_RL)
    # maximum = np.max(bias_transformer_single_variable_RL)
    # median = np.median(bias_transformer_single_variable_RL)
    # mean = np.mean(bias_transformer_single_variable_RL)
    # std_dev = np.std(bias_transformer_single_variable_RL)
    # # 打印结果，保留两位小数
    ...
    plt.savefig(resluts_path + 'RECO 0702' + '.png', dpi=300)
    # Display the plot
    plt.show()
