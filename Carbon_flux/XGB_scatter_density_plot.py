...
def interpolation_quality(DL_Y_predict,DL_Y_true,inter_mask,DL_y_qa,XGB_NEE_predict,XGB_GPP_predict,XGB_RECO_predict,is_inter = 0):
    ...
    # 第二个值：地面站点的RECO-GPP
    # Ground_RECO_GPP = XGB_Y_true_array[:, 2] - XGB_Y_true_array[:, 1] # 64425
    ...
    # # # 第五个值 单个随机森林预测的 NEE
    XGB_NEE = XGB_NEE_predict[index2_flattened]
    # 第六个值： 单个随机森林预测的RECO CPP
    XGB_GPP = XGB_GPP_predict[index2_flattened]
    XGB_RECO = XGB_RECO_predict[index2_flattened]
    return DL_NEE,DL_GPP,DL_RECO,Ground_NEE,Ground_GPP,Ground_RECO,XGB_NEE, XGB_GPP, XGB_RECO
...
if __name__ == "__main__":
    ...
    DL_y_qa[:,:,3]
    # XGB three
    XGB_NEE_predict = np.load(inputdata_path + 'XGB_RL_NEE_predict_1.npy', allow_pickle=True)
    XGB_NEE_true = np.load(inputdata_path + 'XGB_RL_NEE_true_1.npy', allow_pickle=True)

    XGB_GPP_predict = np.load(inputdata_path + 'XGB_RL_GPP_predict_1.npy', allow_pickle=True)
    XGB_GPP_true = np.load(inputdata_path + 'XGB_RL_GPP_true_1.npy', allow_pickle=True)

    XGB_RECO_predict = np.load(inputdata_path + 'XGB_RL_RECO_predict_1.npy', allow_pickle=True)
    XGB_RECO_true = np.load(inputdata_path + 'XGB_RL_RECO_true_1.npy', allow_pickle=True)

    # 第一组值:插值与不插值的一起
    DL_NEE_0, DL_GPP_0, DL_RECO_0, Ground_NEE_0, Ground_GPP_0, Ground_RECO_0,XGB_NEE_0, XGB_GPP_0, XGB_RECO_0 = interpolation_quality(
        DL_Y_predict, DL_Y_true, inter_mask, DL_y_qa,XGB_NEE_predict,XGB_GPP_predict,XGB_RECO_predict, is_inter=0)
    DL_NEE_1, DL_GPP_1, DL_RECO_1, Ground_NEE_1, Ground_GPP_1, Ground_RECO_1,XGB_NEE_1, XGB_GPP_1, XGB_RECO_1 = interpolation_quality(
        DL_Y_predict, DL_Y_true, inter_mask, DL_y_qa,XGB_NEE_predict,XGB_GPP_predict,XGB_RECO_predict, is_inter=1)
    DL_NEE_2, DL_GPP_2, DL_RECO_2, Ground_NEE_2, Ground_GPP_2, Ground_RECO_2,XGB_NEE_2, XGB_GPP_2, XGB_RECO_2 = interpolation_quality(
        DL_Y_predict, DL_Y_true, inter_mask, DL_y_qa,XGB_NEE_predict,XGB_GPP_predict,XGB_RECO_predict, is_inter=2)
    ...
    Y_variables = ['XGB_NEE_0', 'XGB_GPP_0', 'XGB_RECO_0', 'XGB_NEE_1', 'XGB_GPP_1', 'XGB_RECO_1', 'XGB_NEE_2', 'XGB_GPP_2',
                   'XGB_RECO_2']
    ...
    Y_labels = ['XGB NEE overall', 'XGB GPP overall', 'XGB RECO overall', 'XGB NEE with no-interpolation', 'XGB GPP with no-interpolation', 'XGB RECO with no-interpolation',
                   'XGB NEE with no-interpolation and y-quality', 'XGB GPP with no-interpolation and y-quality', 'XGB RECO with no-interpolation and y-quality']
    ...
    for i in range(9):
        ...
        x = globals()[x_variable] # Get the data for the XGB variable
    ...
    base_name = 'Results of XGBoost using reflectances, LAI, FPAR and meteorological variables'
    ...
    plt.savefig(resluts_path+base_name +'3'+ '.png', dpi=600)
    plt.show()
