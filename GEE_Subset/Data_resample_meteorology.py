...
# 针对气象变量数据
def read_multiband_mete(imagepath, point, bandlist):
    mete_datasets = []
    for file in bandlist:
        # 将数据集裁剪为50*50大小
        dataset, im_proj, im_geotrans = resubset_values(point, file)  # 这里需要定义resubset_values函数来处理提取数据，您可以根据您的需要来实现它
        # 针对不同变量进行处理
        ## 太阳辐射变量需要转换单位，参考网址 https://developers.google.com/earth-engine/datasets/catalog/ECMWF_ERA5_LAND_MONTHLY_AGGR#bands 对这个变量 surface_net_solar_radiation_sum 的解释
        # 在这里需要完善判断条件，如果 file 包含 'surface_solar_radiation_downwards_sum'，则对 mete_dataset 的值进行转换
        # if 'surface_solar_radiation_downwards_sum' in file:
        #     mete_dataset = np.where(dataset != -9999, dataset / 86400, dataset)  # 对不等于-9999的数值进行换算，等于-9999的保持原值
        # else:
        #     mete_dataset = dataset
        mete_dataset = dataset
        mete_datasets.append(mete_dataset)
    # 将列表转为 ndarray
    re_datasets = np.array(mete_datasets)
    return re_datasets, im_proj, im_geotrans
...
def remove_clouds(point,reflectance_file_path,quality_file_path):
    # 将\\换为/
    qa_data,qa_proj,qa_geotrans = read_tif(point,quality_file_path)
    re_data,re_proj,re_geotrans = read_tif(point,reflectance_file_path)
    # 将qa_data中为1的位置视为云像素位mask，然后将对应位置的re_data数据标记为-9999
    print(qa_data,re_data)
    # 标记值为1或-9999的位置为云像素
    cloud_mask = (qa_data == 1) | (qa_data == -9999)
...
if __name__ == '__main__':
    ...
    # site_image存储根路径
    site_image_path = 'E:/Carbon_flux/data/modis/Sites_image_ERA5_LAND_DAILY_AGGR'
    # 进入site目录
    ...
    for i in range(0, 2):
        ...
        for j in range(len(site_year_list)):
            ...
            for k in range(0,len(year_imag_list)):
                ...
                # bandlist = ['surface_solar_radiation_downwards_sum','temperature_2m',
                #             'volumetric_soil_water_layer_1','volumetric_soil_water_layer_2','volumetric_soil_water_layer_3','volumetric_soil_water_layer_4']
                bandlist = os.listdir(imagepath)
                meteorology_doy_list = []
                # 读取该路径下的所有波段
                meteorology_datasets, im_proj, im_geotrans = read_multiband_mete(imagepath, point, bandlist)
                 # 将一天的7个波段的影像添加到一个list
                x_year_list.append(meteorology_datasets)# 将这一年的每一天的7个波段的影像添加到一个list
                # 返回影像的年份和doy
                ...
                # 将re_doy_list转为array
                re_doy_mulbandarray = np.array(meteorology_doy_list)
    ...
    np.save(inputdata_path + 'x_image_array_meteorology_3.npy', x_image_array)
