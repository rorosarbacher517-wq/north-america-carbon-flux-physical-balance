...
if __name__ == '__main__':
    sites_df = pd.read_csv("E:/China_forest/data/Inventory Dataset/China_landsat_nodownload.csv")
    # df = pd.read_csv('C:/Users/wuth/Desktop/image_df.csv')
    # print(df.head(5))
    ## 创建影像存储的根目录
    sites_image_root_path = 'E:/China_forest/data/Image'
    nodata_list = []
    nodata_df = pd.DataFrame()
    for m in range(0,len(sites_df)):
        sites_image_path_name = str(sites_df['Site_Id'][m])
        ...
        # year_list = []
        year = sites_df['Investigate year'][m]
    ...
    nodata_df.to_csv('E:/China_forest/data/Image/nodata_df_0_50_10.csv')
