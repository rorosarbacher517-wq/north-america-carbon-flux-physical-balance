...
def read_multiband(imagepath,point,bandlist):
    ...
    for keyword in bandlist:
        file = glob.glob(f'{imagepath}/*{keyword}*')
        # 将dataset提取为50*50大小
        dataset, im_proj, im_geotrans = resubset_values(point, file[0])
...
if __name__ == '__main__':
    ...
    for i in range(0, len(ref_sites_list)):
        ...
        # 进入lai站点目录下的各个年份目录
        lai_sites_year_path = os.path.join(lai_image_path, lai_sites_list[i])
        # 进入ref_site_year list
        lai_sites_year_list = os.listdir(lai_sites_year_path)
        ref_site_lists = []
        lai_site_lists = []
        flux_site_lists = []
        #进入ref_site_year_list lai_site_year_list
        ...
        # for m,n in zip(range(len(ref_sites_year_list)), range(len(lai_sites_year_list))):
            ...
            time_year_list = []
            site_year_list = []
            for k in range(0,len(ref_site_year_doy_list)):
                ...
                ref_bandlist = ['Reflectance_Band1', 'Reflectance_Band2', 'Reflectance_Band3', 'Reflectance_Band4', 'Reflectance_Band5', 'Reflectance_Band6', 'Reflectance_Band7',
                            'Quality_Band1', 'Quality_Band2', 'Quality_Band3', 'Quality_Band4', 'Quality_Band5', 'Quality_Band6', 'Quality_Band7']
                ...
                lai_site_year_doy_path = os.path.join(lai_site_year_path, ref_site_year_doy_date +'_MCD15A3H')
                ...
                if not os.path.isdir(lai_site_year_doy_path):
                    ...
                    lai_year_list.append(lai_datasets)
                # 返回影像的年份和doy
                year, doy, Date = out_day_by_date(ref_site_year_doy_date)
    ...
    np.save(inputdata_path + 'x_image_array_modis_0_270.npy', ref_array)
    np.save(inputdata_path + 'x_lai_array_modis_0_270.npy', lai_array)
    np.save(inputdata_path + 'y_meteorological_array_flux_0_270.npy', flux_array)