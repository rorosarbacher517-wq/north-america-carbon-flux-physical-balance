...
def get_product_band(year):
    ...
    #     # band_list = ['Fpar','Lai','FparLai_QC',]
    #     band_list = ['Fpar', 'Lai', 'FparLai_QC', 'FparExtra_QC','FparStdDev','LaiStdDev']
    # else:
    #     print('the data is not available')
    # 下载气象数据 根据year判断产品类型和band
    if year > 1999 and year < 2022:
        product = ['ECMWF/ERA5_LAND/DAILY_AGGR']
        band_list = ['surface_solar_radiation_downwards_sum', 'temperature_2m', 'volumetric_soil_water_layer_1',
...
if __name__ == '__main__':
    ...
    # nodata_df = pd.read_csv("E:/Carbon_flux/data/modis/lai_data_nodownload_231225.csv")
    # #
    # # # 将两个df merge起来
    # nodata_contact_df = pd.merge(nodata_df, sites_df, on='Site_Id', suffixes=('', '_y'), how='left')
    # print(nodata_contact_df)
    # nodata_contact_df.to_csv("E:/Carbon_flux/data/modis/lai_nodata_df_270_231225.csv")
    sites_df = pd.read_csv('E:/Carbon_flux/data/modis/Sites_meteorology_nodownload.csv')
    # df = pd.read_csv('C:/Users/wuth/Desktop/image_df.csv')
    # df = pd.read_csv("E:/Carbon_flux/data/input_data/MODIS_Sites_test.csv")
    # print(df.head(5))
    ## 创建影像存储的根目录
    # sites_image_root_path = 'E:/Carbon_flux/data/modis/sites_image_modis_fpar_lai_mcd15a3h'
    ...
    for m in range(0, len(sites_df)):
        ...
        df = gee_subset.gee_subset(product=product,
                                   ...
                                   pad=1)
        # print(df.head())
        # df.to_csv("E:/Carbon_flux/data/input_data/MODIS_Sites_test_1.csv")
        if df is None:
            ...
            for group_name, group_df in grouped:
                ...
                print(group_df)
                # 将image_numpy转为tif
                image_numpy = utility.get_image_fast(group_df, fields=band, dtype=np.uint16)
                ...
                for j in range(num_bands):
                    ...
                    image_name = group_name + '_' + product.replace("/", "_")[6:]
    ...
    nodata_df.to_csv('E:/Carbon_flux/data/nodata/lai_nodata_df_0_50_10.csv')