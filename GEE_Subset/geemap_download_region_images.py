...
if __name__ == '__main__':
    ...
    output_base_dir = "E:/Paper code/code/Vegetation_productivity_prediction/Region_prediction/data/original_images/gadm41_CAN_1"

    # Define the UMC feature collection
    UMC_input_path = "E:/Paper code/code/Vegetation_productivity_prediction/Region_prediction/data/geoshp/gadm41_CAN_1-20240306T074436Z-001/gadm41_CAN_1"
    ...
    # for state in states_list[2:]:
    #     # Create a subdirectory for the current state if it doesn't exist
    #     state_dir = os.path.join(output_base_dir, state)
    #     os.makedirs(state_dir, exist_ok=True)
    #
    #     roi_bounds = UMC.filter(ee.Filter.eq('NAME_1', state)).geometry().bounds()
    ...
    #             # composite = image.clip(UMC.filter(ee.Filter.eq('NAME_1', state)).geometry().bounds())
    #             out_tif = os.path.join(product_dir, f"{current_date_str}.tif")
    #
    #             geemap.download_ee_image(
    #                 image=composite,
