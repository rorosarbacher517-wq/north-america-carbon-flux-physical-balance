...
# 本版本是在初始版本的基础上将获取到的一个州的影像分块输出，同一个块同一个产品不同天的输出到一个文件夹
    ...
    for state in states_list[8:9]:
        ...
        for i, product in enumerate(product_list):
            ...
            while current_date <= end_date:
                ...
                # composite = image.clip(roi_bounds)
                ...
                #
                # # 生成用于切割影像的网格features
                # features = geemap.fishnet(roi_bounds, rows_in_tiles, cols_in_tiles, delta=0)

                # geemap.download_ee_image_tiles(composite,features,product_dir,prefix=current_date_str+'_',crs="EPSG:4326", scale=500,resampling='near',shape=(32,32))
