...
# Function to read files in blocks
def read_in_blocks(file_path, state_output_path,product_name,date,geoinfo_base_dir,state_name):
    ...
    with rasterio.open(file_path) as src:
        for i in range(0, src.height, block_size):
            for j in range(0, src.width, block_size):
                ...
                block_index_i = i // block_size
                block_index_j = j // block_size
                ...
                with rasterio.open(block_output_path, 'w', driver='GTiff', height=block_size, width=block_size,
                                   ...
                                   transform=from_origin(src.bounds.left + j * src.res[0],
                                                         src.bounds.top - i * src.res[1], src.res[0],
                                                         src.res[1])) as dst:
...
if __name__ == '__main__':
    ...
    nation_path = os.path.join(region_dir, nation_list[1])
    ...
    for s in range(0,1):
        ...
        for directory in product_directories:
            ...
            for f in range(0,len(os.listdir(directory))):
                file = filelist[f]
                file_path = os.path.join(directory, file)
                date = file.split('.')[0]
                read_in_blocks(file_path, state_output_path,product_name,date,geoinfo_base_dir,state_name)
                print(file)
