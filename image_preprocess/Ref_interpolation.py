...
# 通过3*3窗口内的有效值进行线性插值
def interpolate_window(interpolated_b_array):
    for b in range(interpolated_b_array.shape[0]):
        if b >= 0 and b <= 6:
            bb = b + 7
        else:
            bb = b
        window = interpolated_array[i, j, bb, :, :]
        if (0 in window) and (1 in window):
            # window[window != 0] = -9999  # Convert non-zero values to -9999
            window[window == 1] = np.nan  # Convert -9999 to NaN
            ...
            if len(non_nan_indices) > 1 and len(non_nan_indices) < 9:
                ...
                interpolated_b_array[b, nan_indices[:, 0], nan_indices[:, 1]] = interpolated_values.flatten()  # Assign the interpolated values to the NaN positions
            elif len(non_nan_indices) == 1:
                non_nan_value = interpolated_b_array[b, non_nan_indices[0, 0], non_nan_indices[0, 1]]
                interpolated_b_array[b, nan_indices[:, 0], nan_indices[:, 1]] = non_nan_value  # Replace NaN values with the non-NaN value
            elif len(non_nan_indices) == 9:
                interpolated_array[b, :, :] = interpolated_array[b, :, :]
        else:
            interpolated_array[b, :, :] = interpolated_array[b, :, :]  # Update the original array with interpolated values
...
if __name__ == "__main__":
    ...
    ref_array = np.load(inputdata_path + 'x_ref_array_modis_0_270_mark.npy', allow_pickle=True)
    ...
    for i in range(interpolated_array.shape[0]):
        ...
        for j in range(interpolated_array.shape[1]):
            interpolated_array[i, j, :, :, :] = interpolate_window(interpolated_array[i, j, :, :, :])
            ...
            for jj in range(interpolated_array.shape[3]):  # 遍历第二个维度
                for kk in range(interpolated_array.shape[4]):  # 遍历第三个维度
                    ...
                    if (0 in mark_values) and (1 in mark_values):
                        ...
                        interpolate_indices = np.where(interpolated_array[yy, :, bb, jj, kk] == 1)[0]
    ...
    # 保存插值后的数据
    np.save(inputdata_path + 'x_ref_array_modis_0_270_mark_interpolation.npy', interpolated_array)
