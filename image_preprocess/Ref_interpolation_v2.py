####本版本是在Ref_interpolation版本的基础上，改动了插值的部分：用质量好的像素值去替换质量差的或为-9999的像素
...
#                 interpolated_b_array[b, :, :] = interpolated_b_array[b, :, :]
#         else:
#             interpolated_b_array[b, :, :] = interpolated_b_array[b, :, :]  # Update the original array with interpolated values
#
#     return interpolated_b_array
...
if __name__ == "__main__":
    ...
    check_imagemaxmin(interpolated_array)
    # 保存插值后的数据
    np.save(inputdata_path + 'x_ref_array_modis_0_270_mark_interpolation_v3.npy', interpolated_array)
