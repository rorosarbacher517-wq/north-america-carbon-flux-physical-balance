...
# 通过3*3窗口内的有效值进行线性插值
def interpolate_window(window, interpolated_b_array):
...
if __name__ == "__main__":
    ...
    for i in range(interpolated_array.shape[0]):
        ...
        for j in range(interpolated_array.shape[1]):
            ...
            if (0 in window) and (1 in window):
                interpolated_array[i, j, :, :, :] = interpolate_window(window,interpolated_array[i, j, :, :, :])
            else:
        ...
        for jj in range(interpolated_array.shape[3]):  # 遍历第二个维度
            for kk in range(interpolated_array.shape[4]):  # 遍历第三个维度
                ...
                if (0 in mark_values) and (1 in mark_values):
                    ...
                    interpolate_indices = np.where(interpolated_array[yy, :, 2, jj, kk] == 1)[0]
                    for ii in range(interpolated_array.shape[2]):# 遍历第一个维度
                        if len(mark_valid_indices) > 1 and len(mark_valid_indices) < 366:
                            ...
                            # 这里需要针对每个波段分别建立插值函数
                            f = interpolate.interp1d(mark_valid_indices, interpolated_array[yy, :, ii, jj, kk][mark_valid_indices], kind='linear',bounds_error=False,
    ...
    # check_imagemaxmin(interpolated_array)
    np.save(inputdata_path + 'x_lai_array_modis_0_270_mark_interpolation.npy', interpolated_array)
